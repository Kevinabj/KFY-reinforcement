...
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows.meta, calc, decorations.pathmorphing, backgrounds, positioning, shadows, shadows.blur}
\input{packages}
\input{macros}
...
% Algorithm identity colors (matching the matplotlib tab10 hues used in the plots)
...
% ====================
% Algorithm card: a colored left rule + body
% ====================
...
% ====================
% Key-number badge (a colored disc with a big number and a one-line caption)
% ====================
\newcommand{\keynum}[3]{%
  \begin{tikzpicture}
...
% ====================
% Environment cartoons (TikZ) -- BIGGER and more detailed
% ====================
...
\begin{tikzpicture}[scale=0.95, every node/.style={font=\scriptsize}]
  ...
  % cart with subtle gradient look
...
\begin{column}{\colwidth}
    ...
    % ---------- Environments with bigger TikZ cartoons ----------
    ...
    % ---------- Bar charts ----------
    \begin{block}{5. Cost, sample efficiency, model size}
        \vspace{-0.3em}
...
\begin{column}{\colwidth}
    ...
    \begin{block}{A. Learning curves: one figure per environment}
        ...
        \begin{minipage}[t]{0.49\colwidth}
            ...
            \small \textbf{Pendulum.} \hl{SAC and TD3: $\sim 13\times$ more sample-efficient than PPO} (SAC threshold at 7.4k vs PPO 95k).
        \end{minipage}\hfill
        \begin{minipage}[t]{0.49\colwidth}
            \includegraphics[width=\linewidth]{../plots/per_env/MountainCarContinuous-v0.pdf}\\
            \small \textbf{MCC, sparse.} \hl{Only PPO ever finds the goal} (1/3 seeds, $-2.5$); SAC $-0.8$, TD3 $-9.1$.
        \end{minipage}
    \end{block}

    % ---------- All four ablations consolidated into ONE alert block ----------
    \begin{alertblock}{B. Pre-registered ablations: turning the canonical knobs}
        ...
        \begin{minipage}[t]{0.46\colwidth}
            ...
            Auto, 0.05, 0.20: three failure modes, \hl{0 goal discoveries / 9 seeds}.
        \end{minipage}\hfill
        \begin{minipage}[t]{0.46\colwidth}
            \centering
            ...
            $\sigma\in\{0.1, 0.3, 0.5\}$: \hl{more noise is worse, not better}.
        \end{minipage}
        ...
        \begin{minipage}[t]{0.46\colwidth}
            ...
            $\tau\in\{100, 1000, 5000\}$: \hl{U-shape}, $\tau{=}1000$ at the edge of stability (explains seed-bimodality).
        \end{minipage}\hfill
        \begin{minipage}[t]{0.46\colwidth}
            \centering
            \includegraphics[width=\linewidth]{../plots/ablations/ppo_clip_ratio.pdf}\\
            \small \textbf{\algotag[ppocol]{PPO} clip ratio, Pendulum.} \\
            $\epsilon\in\{0.1, 0.2, 0.3\}$: 0.1 too conservative; 0.2 \& 0.3 tied.
        \end{minipage}
    ...
    % ---------- Conclusion as EXAMPLE block ----------
    \begin{exampleblock}{C. Practical rule of thumb}
        \begin{itemize}
            \itemsep0.1em
    ...
    % ---------- Project artefacts ----------
    \begin{block}{Project artefacts}
        \small
        \begin{itemize}
            \itemsep0.1em
            \item \textbf{Code}: PyTorch, no \texttt{stable-baselines3}. Per-(algo, env) YAML configs under \texttt{configs/}.
            \item \textbf{Plots}: \texttt{plots/main\_grid.pdf}, per-env curves, seed grids, ablations, and the bar charts shown here.
            \item \textbf{Numbers}: full scoreboard with bootstrap CIs in \texttt{plots/scoreboard.csv} and \texttt{scoreboard.tex}.
        \end{itemize}
    \end{block}
