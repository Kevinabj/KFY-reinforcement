...
\begin{column}{\colwidth}
    ...
    \begin{block}{6. Limitations \& open questions}
        \begin{itemize}
            \itemsep0.05em
            \item \textbf{Seed count.} 3 per (algo, env); more would tighten CIs at super-linear cost. Bootstrap CIs are wide enough to keep claims honest.
            \item \textbf{Network widths fixed} at canonical paper defaults: $(64,64)$ for DQN/PPO, $(256,256)$ for SAC/TD3. The actor-parameter gap is a width choice, not an algorithmic property.
            \item \textbf{Sparse reward $=$ MCC only.} Generalizing the headline needs a second sparse-reward benchmark, e.g.\ \texttt{Reacher} with a binary reward or a sparsified \texttt{Pendulum}.
            \item \textbf{No exploration baselines.} Natural next step: add RND or NoisyNet on top of SAC/TD3 on MCC; does directed exploration close the gap local entropy/noise cannot?
        \end{itemize}
    \end{block}
