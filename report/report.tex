...
\item On sparse-reward continuous control (MountainCarContinuous), the off-policy methods fail to escape a lazy local optimum, and only PPO ever discovers the goal.
\end{enumerate}
...
We model each environment as a discrete-time Markov decision process (MDP) $\mathcal{M} = (\mathcal{S}, \mathcal{A}, P, r, \gamma, \rho_0)$ with state space $\mathcal{S}$, action space $\mathcal{A}$, transition kernel $P(s' \mid s, a)$, reward $r(s, a)$, discount $\gamma \in [0, 1)$, and initial state distribution $\rho_0$. A policy $\pi(a \mid s)$ induces a discounted return $J(\pi) = \mathbb{E}_{\pi}\!\left[\sum_{t \geq 0} \gamma^{t} r(s_t, a_t)\right]$, which all four algorithms aim to maximize.
...
We pre-registered four ablations before running any sweep, one per algorithm. As of this report's compile time the full sweeps are still running on our reduced 100k-step ablation protocol; the figures below are placeholders and will be replaced by the per-sweep plots in \texttt{plots/ablations/} when the runs complete.

\textbf{DQN, CartPole-v1: target-sync period.} Sweep $\{100, 1000, 5000\}$. Hypothesis: $100$ destabilizes learning (almost no target), $5000$ slows convergence within the 100k budget; $1000$ is the sweet spot. [TODO: insert plot once available.]

\textbf{PPO, Pendulum-v1: clip ratio.} Sweep $\{0.1, 0.2, 0.3\}$. Hypothesis: $0.2$ is the canonical default, $0.1$ is too conservative on dense-reward continuous control, $0.3$ erodes the soft trust region. [TODO: insert plot once available.]

\textbf{SAC, MountainCarContinuous-v0: temperature schedule.} Auto-$\alpha$ (target entropy $-|\mathcal{A}|$) versus fixed $\alpha \in \{0.05, 0.2\}$. Hypothesis: sparse reward stresses exploration; the auto schedule should keep entropy high until the policy locks on. Given our Section~5 finding that SAC never reaches the goal in 300k steps even with auto-$\alpha$, we expect the fixed-$\alpha$ baselines to fare no better.

\textbf{TD3, MountainCarContinuous-v0: exploration noise.} Sweep $\sigma_{\text{exp}} \in \{0.1, 0.3, 0.5\}$. Hypothesis: $0.1$ is too quiet for the sparse-reward task; $0.5$ may finally reach the goal occasionally. This is the cleanest test of our Section~5 claim that the off-policy methods' exploration is locally myopic on this environment.
...
\textbf{(c) Which one scales better for continuous actions?} There is no single winner. SAC and TD3 dominate on dense-reward continuous control (Pendulum, where both are $\sim 12\times$ more sample-efficient than PPO). PPO is the only one that solves our sparse-reward continuous task (MountainCarContinuous), where SAC and TD3 are blocked by their locally myopic exploration. Practically: pick SAC or TD3 first; if the reward is sparse and exploration is the bottleneck, switch to PPO or augment with explicit exploration bonuses.
